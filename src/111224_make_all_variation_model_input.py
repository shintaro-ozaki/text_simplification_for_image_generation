...
def make_prompt_for_caption_reference_description_and_entity(caption_reference_description, entity, abstract):
  prompt = f"""
Please generate an image using the following information.
...
def make_all_variation_model_input(wit_data):
  """
  ...
  * (caption_reference_description+entity) -> summary
  """
  datalist = []
  output_path = project_root / 'data' / 'wit' / 'en.wit.2k.prompt.jsonl'
  for i, line in enumerate(wit_data):
    ...
    try:
      ...
      title = line['page_title'].replace('/', '_').replace(':', '_')
      ...
      # caption_reference_descriptionに入っているentityのみを抽出
      entity_in_caption: list[str] = [entity for entity in combined_entities if entity in caption_reference_description]
      ...
      prompt2 = make_prompt_for_caption_reference_description_and_entity(caption_reference_description, entity_in_caption, abstracts)
...
if __name__ == "__main__":
  ...
  make_all_variation_model_input(wit_data)
