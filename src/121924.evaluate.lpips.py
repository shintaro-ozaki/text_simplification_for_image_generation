  ...
  for pred_file, ref_file in tqdm(zip(pred_files, ref_files)):
    try:
      ...
      img0 = Image.fromarray(
          (img0 * 255).astype(np.uint8)) if img0.max() <= 1 else Image.fromarray(
              img0.astype(np.uint8))
      img1 = Image.fromarray(
          (img1 * 255).astype(np.uint8)) if img1.max() <= 1 else Image.fromarray(
...
if __name__ == "__main__":
  args = parse_args()
  calculate_lpips(
      Path(args.dir0), Path(args.dir1), Path(args.out), args.use_gpu)
